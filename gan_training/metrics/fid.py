...
def get_activations(images, sess, batch_size=200, verbose=False):
    ...
    if images.shape[3] == 1:
        ...
        images = np.stack([images,images,images],3)
...
def compute_fid_from_npz(path):
    ...
    with np.load(path) as data:
        ...
        name = None
        for name in ['imagenet', 'cifar', 'places','mnist','celebA','fashion', 'VGG','stl10']:
            if name in path: 
                real_imgs = name
                break
...
precomputed_stats = {
    ...
    'mnist':
    'output/mnist_gt_stats.npz',
    'celebA':
    'output/celebA_gt_stats.npz',
    'fashion':
    'output/fashion_gt_stats.npz',
    'VGG':
    'output/VGG_64_gt_stats.npz',
    'stl10':
    'output/stl_gt_stats.npz'
...
def compute_stats(exp_path):
    ...
    if 'stl10' in exp_path and not os.path.exists(precomputed_stats['stl10']):
        with np.load('output/stl_gt_imgs.npz') as data_real:
            real_imgs = data_real['real']
    ...
    if 'VGG' in exp_path and not os.path.exists(precomputed_stats['VGG']):
        with np.load('output/VGG_64_gt_imgs.npz') as data_real:
            real_imgs = data_real['real']
            print('loaded real VGG images', real_imgs.shape)
        inception_path = check_or_download_inception(None)
        create_inception_graph(inception_path)
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            m, s = calculate_activation_statistics(real_imgs, sess)
        np.savez(precomputed_stats['VGG'], m=m, s=s)

    if 'mnist' in exp_path and not os.path.exists(precomputed_stats['mnist']):
        with np.load('output/mnist_gt_imgs.npz') as data_real:
            real_imgs = data_real['real']
            print('loaded real mnist images', real_imgs.shape)
        inception_path = check_or_download_inception(None)
        create_inception_graph(inception_path)
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            m, s = calculate_activation_statistics(real_imgs, sess)
        np.savez(precomputed_stats['mnist'], m=m, s=s)

    if 'fashion' in exp_path and not os.path.exists(precomputed_stats['fashion']):
        with np.load('output/fashion_gt_imgs.npz') as data_real:
            real_imgs = data_real['real']
            print('loaded real fashion mnist images', real_imgs.shape)
        inception_path = check_or_download_inception(None)
        create_inception_graph(inception_path)
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            m, s = calculate_activation_statistics(real_imgs, sess)
        np.savez(precomputed_stats['fashion'], m=m, s=s)

    if 'celebA' in exp_path and not os.path.exists(precomputed_stats['celebA']):
        with np.load('output/celebA_gt_imgs.npz') as data_real:
            real_imgs = data_real['real']
            print('loaded real celebA images', real_imgs.shape)
        inception_path = check_or_download_inception(None)
        create_inception_graph(inception_path)
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            m, s = calculate_activation_statistics(real_imgs, sess)
        np.savez(precomputed_stats['celebA'], m=m, s=s)
